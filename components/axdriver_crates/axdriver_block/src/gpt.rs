...
impl FromStr for GptPartitionName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for ch in s.chars() {
            ...
            let code_unit =
                u16::try_from(u32::from(ch)).map_err(|_| GptPartitionNameFromStrError::InvalidChar)?;
...
fn read_bytes<T: BlockDriverOps>(
    ...
    while !dst.is_empty() {
        ...
        let within_block = usize::try_from(offset % block_size_u64).map_err(|_| DevError::BadState)?;
        ...
        dst = rest;
        offset = checked_add(offset, u64::try_from(copy_len).map_err(|_| DevError::BadState)?)?;
    }
    ...
    F: FnMut(usize, &GptPartitionEntry) -> bool,
{
    ...
    let entry_count =
        usize::try_from(primary_header.number_of_partition_entries).map_err(|_| DevError::BadState)?;
